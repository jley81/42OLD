...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/42.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specific to 42 / RPC client
            std::string strUsage = _("42 version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  42 [options]                     " + "\n" +
                  "  42 [options] <command> [params]  " + _("Send command to -server or 42") + "\n" +
                  "  42 [options] help                " + _("List commands") + "\n" +
                  "  42 [options] help <command>      " + _("Get help for a command") + "\n";
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -pid=<file>            " + _("Specify pid file (default: 42.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 24242 or testnet: 42424)") + "\n" +
...
bool AppInit2()
{
    ...
    // 42: Keep irc seeding on by default for now.
//    if (fTestNet)
//    {
...
//    }
    ...
    // Make sure only a single 42 process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  42 is probably already running."), GetDataDir().string().c_str()));
    ...
    // requested to kill 42-qt during the last operation. If so, exit.
